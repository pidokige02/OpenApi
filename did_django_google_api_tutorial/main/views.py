...
def route(request):
	...
	# project 의 template directory 의 main directory 의 route.html file 을 읽어온다.
...
'''
def map(request):
	# 사용자가 URL에 쿼리 파라미터로 lat_a 값을 포함하여 요청했을 때,
	# 그 값이 request.GET.get("lat_a", None)을 통해 가져옴.
...
# map.html에서 {{ directions.<key> }} 형식으로 값을 출력하거나,
# 리스트를 순회하는 방식으로 경로의 세부 정보를 표시
